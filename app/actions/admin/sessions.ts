...
const SESSION_BUCKET = 'sessions'
const ALLOWED_MIME = ['image/jpeg', 'image/png', 'image/webp', 'image/gif']
const MAX_FILE_SIZE = 10 * 1024 * 1024 // 10 MB

async function ensureBucket() {
  const { data: buckets } = await supabaseAdmin.storage.listBuckets()
  if (!buckets?.find((b) => b.name === SESSION_BUCKET)) {
    const { error } = await supabaseAdmin.storage.createBucket(SESSION_BUCKET, {
      public: true,
...
export async function uploadSessionImageAction(formData: FormData): Promise<{ url: string }> {
  ...
  if (error) {
    console.error('[sessions] upload error:', error.message)
    throw new Error('Upload failed')
  }

  const { data: urlData } = supabaseAdmin.storage.from(SESSION_BUCKET).getPublicUrl(fileName)
